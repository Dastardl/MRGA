...
//Wheels vars
#define rightPwmPort 9
#define rightInputPort 6
#define leftPwmPort 10
#define leftInputPort 7
...
void setup()
{
    ...
    pinMode(rightInputPort, OUTPUT);

    pinMode(leftPwmPort, OUTPUT);
    pinMode(leftInputPort, OUTPUT);

    //Set Wheel Direction
    digitalWrite(rightInputPort, LOW);
    digitalWrite(leftInputPort, LOW);
...
int compareAngle;
int waiter;
int nextDirection = 0; //0 = forward, 1 = backward
int beep = 0;
void loop()
{
    if (IS_OK(lidar.waitPoint()))
    {
        ...
        if (count == 0)
        {
            ...
            if (isDriving)
            {
                analogWrite(leftInputPort, LOW);
                analogWrite(rightInputPort, LOW);
                analogWrite(rightPwmPort, 150);
                analogWrite(leftPwmPort, 150);
                beep = 0;
            }
            else
            {
                analogWrite(leftInputPort, HIGH);
                analogWrite(rightInputPort, HIGH);
                analogWrite(rightPwmPort, 100);
                analogWrite(leftPwmPort, 100);
                if (beep == 0)
                    tone(PIEZO_PIN, 2400, 600);
                beep++;
                beep = beep % 5;
            }
